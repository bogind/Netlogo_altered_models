...
patches-own[age generation tree]
breed [seeds seed]
breed [fires fire]
...
to setup
  ...
  set last-fire 0
  create-fires 1 [set hidden? true]
  ;; keep track of how many trees there are
  set initial-trees count patches with [pcolor = green]
  set forest count patches with [pcolor = blue] + initial-trees
  set-default-shape seeds "circle"
;  create-seeds 100 [set color white set size 3]
...
to go
  ...
  if all? patches [pcolor != red] [
    ...
    [sprout-seeds random 2 [set color white set size 2]
    ask seeds[
      spread
      try-to-plant
      ]
  ...
  ask patches with [ pcolor = red ] [ ;; ask the burning trees
    ask neighbors4 with [tree = true ] [ ;; ask their non-burning neighbor trees
    ...
    if all? patches [pcolor != red and count fires = 0 ][
  set last-fire ticks
  ]
  if all? patches [ pcolor != red ] and ticks - last-fire > 50[
    ask fires[die]
    fire-spread
  ]
tick
...
to spread
  ask seeds[
   let  angle random-float 50 - random-float 50
   rt angle
   fd random 7
  ]
end
...
to try-to-plant
  ask seeds [
    if pcolor = red [ask seeds [die]]
    ifelse pcolor = black or pcolor = (red - 3.5) [plant][ask seeds [die]]
  ]
end
...
to plant
    ask seeds[
    ifelse pcolor = black[
      ifelse random 100 < probTOGowInBlack[
        set pcolor generation
        set generation generation + 4
        set tree true
        set i generation
                die
    ][die
    ]][
    ifelse pcolor = red - 3.5[
      ifelse random 100 < probTOGowInBurned[
        set pcolor generation
        set generation generation + 4
         set tree true
          set i generation
...
to fire-spread
  if ticks - last-fire > 50[ask n-of 1 patches with [pcolor != red or pcolor != black][set pcolor red]]
  ask one-of patches
    [sprout-fires 1[set hidden? true]]
  set last-fire ticks
end
@#$#@#$#@
GRAPHICS-WINDOW
...
69
percent burned
(count patches with [shade-of? pcolor red]) / count patches  * 100
1
1
11
...
116
precent new trees
(count patches with [generation = i and tree = true])/ count patches with [tree = true] * 100
17
1
11
...
161
% trees
(count patches with [tree = true]) / count patches  * 100
17
1
11
...
"" ""
PENS
"default" 1.0 0 -13840069 true "" "plot count patches with [tree = true]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count patches with [generation = 2 and tree = true]"
"pen-1" 1.0 0 -8990512 true "" "plot count patches with [generation = 6 and tree = true]"
"pen-2" 1.0 0 -7500403 true "" "plot count patches with [generation = 10 and tree = true]"
"pen-3" 1.0 0 -2674135 true "" "plot count patches with [generation = 14 and tree = true]"
"pen-4" 1.0 0 -955883 true "" "plot count patches with [generation = 18 and tree = true]"
"pen-5" 1.0 0 -6459832 true "" "plot count patches with [generation = 22 and tree = true]"
"pen-6" 1.0 0 -1184463 true "" "plot count patches with [generation = 26 and tree = true]"
"pen-7" 1.0 0 -10899396 true "" "plot count patches with [generation = 30 and tree = true]"
"pen-8" 1.0 0 -13840069 true "" "plot count patches with [generation = 34 and tree = true]"
"pen-9" 1.0 0 -14835848 true "" "plot count patches with [generation = 38 and tree = true]"
"pen-10" 1.0 0 -11221820 true "" "plot count patches with [generation = 42 and tree = true]"
"pen-11" 1.0 0 -13791810 true "" "plot count patches with [generation = 46 and tree = true]"
"pen-12" 1.0 0 -13345367 true "" "plot count patches with [generation = 50 and tree = true]"
"pen-13" 1.0 0 -8630108 true "" "plot count patches with [generation = 54 and tree = true]"
"pen-14" 1.0 0 -5825686 true "" "plot count patches with [generation = 58 and tree = true]"
"pen-15" 1.0 0 -2064490 true "" "plot count patches with [generation = 62 and tree = true]"
...
207
age
max [age] of patches with [ tree = true]
17
1
11
...
<experiments>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="true">
    ...
    <go>go</go>
    <timeLimit steps="100"/>
    <metric>(count patches with [shade-of? pcolor red or shade-of? pcolor black]) / count patches  * 100</metric>
    <metric>(count patches with [generation = i and tree = true])/ count patches with [tree = true] * 100</metric>
    <metric>(count patches with [tree = true]) / count patches  * 100</metric>
    <metric>max [age] of patches with [ tree = true]</metric>
    <metric>last-fire</metric>
    ...
    <enumeratedValueSet variable="probTOGowInBurned">
      <value value="5"/>
      <value value="30"/>
      <value value="50"/>
      <value value="70"/>
    ...
    <enumeratedValueSet variable="probTOGowInBlack">
      <value value="5"/>
      <value value="30"/>
      <value value="50"/>
      <value value="70"/>
