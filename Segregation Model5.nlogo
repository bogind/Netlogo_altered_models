...
turtles-own [
  ...
  steps               ;; number of turn the turtle has been unhappy
]
...
to setup
  ...
  ask n-of (p-hiloni * count turtles) turtles [ set color red
      set my-%-similar-wanted hiloni-wanted]
  ask n-of (p-dati * count turtles with [ color != red ]) turtles [set color green
      set my-%-similar-wanted dati-wanted]
  ask turtles with [(color !=  red) and (color !=  green) ]
      [set color blue
        set my-%-similar-wanted haredi-wanted]
  update-turtles
  update-globals
  reset-ticks
end
...
to go
  if ticks-so-far = 120 [ stop ]
  if all? turtles [happy?] [ stop ]
  move-unhappy-turtles
  update-turtles
  update-globals
  set ticks-so-far ticks-so-far + 1
  growth
  tick
end
...
to update-turtles
  ask turtles with [color = blue][
    ...
    set happy? similar-nearby >= haredi-wanted
  ]
  ...
  ask turtles with [color = red][
    ...
    set happy? similar-nearby >= hiloni-wanted
    ...
    set happy? similar-nearby >= dati-wanted

  ]
end

to growth
  if ticks-so-far mod 12 = 0 and (count turtles < 2500) [
    ask n-of ( 0.02 * count turtles with [ color = red]) turtles with [ color = red ]
       [die]
    ask n-of ( 0.05 * count turtles with [ color = blue]) patches with [count turtles-here = 0][
      sprout 1 [set color blue
      set my-%-similar-wanted haredi-wanted
      set happy? similar-nearby >= haredi-wanted ]
      ]
    ask n-of ( 0.02 * count turtles with [ color = green]) patches with [count turtles-here = 0][
...
475
Hiloni Happy
(count turtles with [(color = red) and (happy?)]) / count turtles with [color = red] * 100
17
1
11
...
475
 Haredi Happy
(count turtles with [(color = blue) and (happy?)]) / count turtles with [color = blue] * 100
17
1
11
...
475
Dati Leumi Happy
(count turtles with [(color = green) and (happy?)]) / count turtles with [color = green] * 100
17
1
11
...
525
Hiloni Unhappy
(count turtles with [(color = red) and (not happy?)]) / count turtles with [color = red]
17
1
11
...
525
Haredi Unhappy
(count turtles with [(color = blue) and (not happy?)]) / count turtles with [color = blue]
17
1
11
...
1000
520
Precent Unhappy Among the Groups
time
%
0.0
105.0
...
"" ""
PENS
"Hiloni unhappy" 1.0 0 -2674135 true "" "plot ( (count turtles with [(color = red) and (not happy?)]) / count turtles with [color = red] )* 100 "
"Haredi unhappy" 1.0 0 -13345367 true "" "plot ( (count turtles with [(color = blue) and (not happy?)]) / count turtles with [color = blue] ) * 100"
"Dati Leumi unhappy" 1.0 0 -13840069 true "" "plot ( (count turtles with [(color = green) and (not happy?)]) / count turtles with [color = green] ) * 100"
